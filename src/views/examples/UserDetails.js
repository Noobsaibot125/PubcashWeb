  ...
  const handleUpdate = async () => {
    try {
      ...
      if (err.response && err.response.status === 400 && err.response.data.message.includes("insuffisants")) {
        // Affichage popup explicite
        alert("⛔ ERREUR : FONDS INSUFFISANTS\n\nVous essayez d'ajouter du solde à cet utilisateur, mais le compte de distribution Admin est vide.\n\nVeuillez recharger le compte Admin.");
      } else {
      ...
      <Container className="mt--7" fluid>
        ...
        <Row>
          ...
          <Col xl="8">
            ...
            {/* SECTION: ENVOYER MESSAGE RAPIDE */}
            <Card className="shadow mt-4">
              <CardHeader className="bg-transparent border-0">
                <h3 className="mb-0 text-primary">
                  <i className="ni ni-send mr-2" />
                  Envoyer un message à {user.nom_utilisateur}
                </h3>
              </CardHeader>
              <CardBody>
                <Form>
                    ...
                    <Col md="12">
                      <FormGroup>
                        ...
                        <Input
                          ...
                          value={formData.msgTitre || ''}
                          onChange={e => setFormData({ ...formData, msgTitre: e.target.value })}
                        />
                      </FormGroup>
                    </Col>
                    <Col md="12">
                      <FormGroup>
                        ...
                        <Input
                          ...
                          value={formData.msgContenu || ''}
                          onChange={e => setFormData({ ...formData, msgContenu: e.target.value })}
                        />
                      </FormGroup>
                    </Col>
                  </Row>
                  <div className="text-right">
                    <Button
                      color="primary"
                      onClick={async () => {
                        if (!formData.msgTitre || !formData.msgContenu) {
                          alert("Veuillez remplir le titre et le message.");
                          return;
                        }
                        if (!window.confirm("Envoyer ce message ?")) return;

                        try {
                          await api.post('/notifications/admin/send', {
                            titre: formData.msgTitre,
                            contenu: formData.msgContenu,
                            target_type: 'single',
                            target_value: user.id
                          });
                          alert("Message envoyé !");
                          setFormData({ ...formData, msgTitre: '', msgContenu: '' });
                        } catch (e) {
